...
if (isset($_POST['alta-reserva'])) {
    // --- Obligatorio: cliente ---
    if (
        empty($_POST['usuario_uuid_hex']) ||
        !preg_match('/^[0-9a-fA-F]{32}$/', (string) $_POST['usuario_uuid_hex'])
    ) {
        $hasError = true;
    } else {
    ...
    $salida_prevista =
        parseDiaHoraToDatetimeOrNull($diaSalida, $horaSalida) ??
        '2030-01-01 00:00:00';

    // --- Opcionales ---
    $vuelo = empty($_POST['vuelo'])
        ? null
        : data_input($_POST['vuelo'], ENT_NOQUOTES);
    $notas = empty($_POST['notes'])
        ? null
        : data_input($_POST['notes'], ENT_NOQUOTES); // notes => notas
    $vehiculo = empty($_POST['vehiculo'])
        ? null
        : data_input($_POST['vehiculo'], ENT_NOQUOTES);
    $matricula = empty($_POST['matricula'])
        ? null
        : data_input($_POST['matricula'], ENT_NOQUOTES);

    $fechaReserva = date('Y-m-d H:i:s');

    // Para generarLocalizador: usar tu función lista
    $dtEntrada = DateTimeImmutable::createFromFormat(
        'Y-m-d H:i:s',
        $entrada_prevista,
    );
    ...
    if (!isset($hasError)) {
        ...
        $stmt->bindValue(
            ':vehiculo',
            $vehiculo,
            $vehiculo === null ? PDO::PARAM_NULL : PDO::PARAM_STR,
        );
        $stmt->bindValue(
            ':matricula',
            $matricula,
            $matricula === null ? PDO::PARAM_NULL : PDO::PARAM_STR,
        );
        ...
        $stmt->bindValue(
            ':vuelo',
            $vuelo,
            $vuelo === null ? PDO::PARAM_NULL : PDO::PARAM_STR,
        );
        $stmt->bindValue(
            ':notas',
            $notas,
            $notas === null ? PDO::PARAM_NULL : PDO::PARAM_STR,
        );
...
if ($codi_resposta == 2) {
    ...
    $placeholderSelected =
        empty($post['usuario_uuid_hex']) && empty($usuarioUuidHexFromPath)
            ? 'selected'
            : '';
    echo '<option value="" disabled ' .
        $placeholderSelected .
        '>Selecciona el client:</option>';
    ...
    foreach ($result as $row) {
        ...
        echo "<option value=\"" .
            htmlspecialchars($uuidHex, ENT_QUOTES) .
            "\" $selected>" .
    ...
    echo '<input type="date" class="form-control" id="diaEntrada" name="diaEntrada" required value="' .
        htmlspecialchars($post['diaEntrada'] ?? '', ENT_QUOTES) .
        '">';
    echo '<div class="form-text">
        <span class="text-danger">*</span> Camp obligatori
        </div>';
    ...
    echo '<input type="time" class="form-control" id="horaEntrada" name="horaEntrada" required value="' .
        htmlspecialchars($post['horaEntrada'] ?? '', ENT_QUOTES) .
        '">';
    echo '<div class="form-text">
        <span class="text-danger">*</span> Camp obligatori
        </div>';
    ...
    echo '<input type="date" class="form-control" id="diaSalida" name="diaSalida" value="' .
        htmlspecialchars($post['diaSalida'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';

    echo '<div class="col-md-3">';
    echo '<label>Hora sortida:</label>';
    echo '<input type="time" class="form-control" id="horaSalida" name="horaSalida" value="' .
        htmlspecialchars($post['horaSalida'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';

    echo '<div class="col-md-3">';
    echo '<label>Vol:</label>';
    echo '<input type="text" class="form-control" id="vuelo" name="vuelo" value="' .
        htmlspecialchars($post['vuelo'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';

    echo '<div class="col-md-3">';
    echo '<label>Model cotxe:</label>';
    echo '<input type="text" class="form-control" id="vehiculo" name="vehiculo" value="' .
        htmlspecialchars($post['vehiculo'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';
    ...
    echo '<input type="text" class="form-control" id="matricula" name="matricula" value="' .
        htmlspecialchars($post['matricula'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';

    echo '<div class="col-md-12">';
    echo '<label>Notes:</label>';
    echo '<input type="text" class="form-control" id="notes" name="notes" value="' .
        htmlspecialchars($post['notes'] ?? '', ENT_QUOTES) .
        '">';
    echo '</div>';
    ...
    echo '<a href="' .
        APP_WEB .
        '/control/clients-anuals/" class="btn btn-dark menuBtn" role="button" aria-disabled="false">Tornar</a>';
