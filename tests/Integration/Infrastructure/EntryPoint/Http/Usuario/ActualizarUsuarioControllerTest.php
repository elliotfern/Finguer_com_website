<?php

declare(strict_types=1);

namespace Tests\Integration\Infrastructure\EntryPoint\Http\Usuario;

use App\Domain\Shared\UsuarioUuid;
use PDO;
use PHPUnit\Framework\Attributes\Group;
use PHPUnit\Framework\TestCase;
use Tests\Support\JwtTestTokenFactory;

#[Group('http')]
final class ActualizarUsuarioControllerTest extends TestCase
{
    private PDO $conn;
    private string $uuidCreado;

    protected function setUp(): void
    {
        parent::setUp();

        $dbName = $_ENV['DB_DBNAME'] ?? '';
        if (!str_contains($dbName, '_test')) {
            $this->fail(
                'PELIGRO: Los tests de integración solo pueden ejecutarse contra una BD de test. ' .
                    "BD detectada: {$dbName}",
            );
        }

        $this->conn = new PDO(
            "mysql:host={$_ENV['DB_HOST']};dbname={$_ENV['DB_DBNAME']}",
            $_ENV['DB_USER'],
            $_ENV['DB_PASS'],
        );
        $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    }

    protected function tearDown(): void
    {
        if (isset($this->uuidCreado)) {
            $uuidBin = UsuarioUuid::fromString($this->uuidCreado)->toBytes();

            $stmt = $this->conn->prepare(
                'DELETE FROM usuarios_perfil WHERE usuario_uuid = :uuid',
            );
            $stmt->bindValue(':uuid', $uuidBin, PDO::PARAM_LOB);
            $stmt->execute();

            $stmt = $this->conn->prepare(
                'DELETE FROM usuarios WHERE uuid = :uuid',
            );
            $stmt->bindValue(':uuid', $uuidBin, PDO::PARAM_LOB);
            $stmt->execute();
        }
        parent::tearDown();
    }

    private function insertarUsuarioDePrueba(string $email): string
    {
        $uuid = UsuarioUuid::generate();

        $stmt = $this->conn->prepare("
            INSERT INTO usuarios (uuid, email, estado, password, tipo_rol, locale)
            VALUES (:uuid, :email, 'activo', NULL, 'cliente', 'es')
        ");
        $stmt->execute([
            ':uuid' => $uuid->toBytes(),
            ':email' => $email,
        ]);

        return $uuid->toString();
    }

    private function apiBaseUrl(): string
    {
        return rtrim(
            $_ENV['API_BASE_TEST'] ?? 'http://127.0.0.1:8000/api',
            '/',
        );
    }

    private function put(string $url, array $payload): array
    {
        $token = JwtTestTokenFactory::generar(
            UsuarioUuid::generate()->toString(),
        );

        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'PUT');
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Accept: application/json',
        ]);
        curl_setopt($ch, CURLOPT_COOKIE, "token={$token}");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

        $body = curl_exec($ch);
        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        // curl_close($ch) eliminado: deprecado desde PHP 8.5, sin efecto desde PHP 8.0

        return [$httpCode, json_decode((string) $body, true)];
    }

    public function test_actualiza_usuario_existente_correctamente(): void
    {
        $this->uuidCreado = $this->insertarUsuarioDePrueba('antes@example.com');

        [$httpCode, $body] = $this->put(
            "{$this->apiBaseUrl()}/usuaris/put?type=usuarios-update",
            [
                'uuid' => $this->uuidCreado,
                'email' => 'despues@example.com',
                'nombre' => 'Nombre Actualizado',
                'tipo_rol' => 'cliente',
                'locale' => 'es',
            ],
        );

        $this->assertSame(200, $httpCode);
        $this->assertSame('success', $body['status'] ?? null);

        $uuidBin = UsuarioUuid::fromString($this->uuidCreado)->toBytes();

        $stmt = $this->conn->prepare(
            'SELECT email FROM usuarios WHERE uuid = :uuid',
        );
        $stmt->execute([':uuid' => $uuidBin]);
        $this->assertSame('despues@example.com', $stmt->fetchColumn());

        $stmt = $this->conn->prepare(
            'SELECT nombre FROM usuarios_perfil WHERE usuario_uuid = :uuid',
        );
        $stmt->execute([':uuid' => $uuidBin]);
        $this->assertSame('Nombre Actualizado', $stmt->fetchColumn());
    }

    public function test_devuelve_404_si_uuid_no_existe(): void
    {
        [$httpCode, $body] = $this->put(
            "{$this->apiBaseUrl()}/usuaris/put?type=usuarios-update",
            [
                'uuid' => UsuarioUuid::generate()->toString(),
                'email' => 'nadie@example.com',
            ],
        );

        $this->assertSame(404, $httpCode);
        $this->assertSame('error', $body['status'] ?? null);
    }

    public function test_devuelve_409_si_email_ya_existe_en_otro_usuario(): void
    {
        $this->uuidCreado = $this->insertarUsuarioDePrueba(
            'usuario1@example.com',
        );
        $otroUuid = $this->insertarUsuarioDePrueba('usuario2@example.com');

        [$httpCode, $body] = $this->put(
            "{$this->apiBaseUrl()}/usuaris/put?type=usuarios-update",
            [
                'uuid' => $this->uuidCreado,
                'email' => 'usuario2@example.com',
            ],
        );

        $this->assertSame(409, $httpCode);
        $this->assertSame('error', $body['status'] ?? null);

        $otroUuidBin = UsuarioUuid::fromString($otroUuid)->toBytes();
        $stmt = $this->conn->prepare('DELETE FROM usuarios WHERE uuid = :uuid');
        $stmt->bindValue(':uuid', $otroUuidBin, PDO::PARAM_LOB);
        $stmt->execute();
    }
}
