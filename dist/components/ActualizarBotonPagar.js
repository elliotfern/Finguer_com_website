...
const tipoReservaSelect = document.getElementById('tipo_reserva'); // Asumiendo que tienes un selector para el tipo de reserva
// Función para verificar si ambos campos de hora están seleccionados
const verificarSelecciones = () => {
...
const validarHorasPorTipoReserva = () => {
    const horaEntradaValue = $('#horaEntrada').val();
    const horaSalidaValue = $('#horaSalida').val();
    const tipoReserva = tipoReservaSelect === null || tipoReservaSelect === void 0 ? void 0 : tipoReservaSelect.value;
    if (!horaEntradaValue || !horaSalidaValue)
        return false;
...
const esHoraValida = (hora, horaMinima, horaMaxima) => {
...
tipoReservaSelect === null || tipoReservaSelect === void 0 ? void 0 : tipoReservaSelect.addEventListener('change', actualizarBotonPagar); // Asegúrate de que este selector existe
