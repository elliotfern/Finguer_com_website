...
type TipoReserva = 'finguer_class' | 'gold_finguer';
...
export const seleccionaHoraTipoReserva = () => {
  ...
  const horasFinguerClass: string[] = [
  ...
  const llenarHoras = (tipo: TipoReserva) => {
  ...
  llenarHoras(tipoReservaSelect.value as TipoReserva);

  // Cambiar las opciones al seleccionar otro tipo de reserva
  tipoReservaSelect.addEventListener('change', (event: Event) => {
    const selectedTipo = (event.target as HTMLSelectElement).value as TipoReserva;
