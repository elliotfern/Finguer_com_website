...
import type { ApiErr, ApiOk, ApiResponse } from '../types/api';
...
export async function transmissioDadesDB<TData = unknown>(
        ...
        if (response.ok && isApiSuccess(parsed)) {
            ...
            if (successBehavior === 'hide') {
                ...
                form.querySelectorAll<HTMLElement>(
                    'input,select,textarea,button,[contenteditable],trix-editor'
            ...
            form.dispatchEvent(
                new CustomEvent<ApiResponse<TData>>('form:success', {
                    detail: parsed,
                })
            );
            return;
    ...
    `;
        ...
        const message =
            error instanceof Error ? error.message : Missatges.error.xarxa;
