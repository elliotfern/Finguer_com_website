...
async function emitirFacturaYObtenerPdfUrl(reservaId: string): Promise<string> {
    ...
    throw new Error(
        errObj.message ?? errObj.error ?? 'Error al generar la factura'
    );
}
...
`;
  ...
  `;
    ...
    const searchInput = searchRow.querySelector(
        '#facturesSearch'
    ) as HTMLInputElement;
    const searchBtn = searchRow.querySelector(
        '#facturesSearchBtn'
    ) as HTMLButtonElement;
    const resetBtn = searchRow.querySelector(
        '#facturesResetBtn'
    ) as HTMLButtonElement;
    const summaryDiv = searchRow.querySelector(
        '#facturesSummary'
    ) as HTMLDivElement;
    const csvBtn = searchRow.querySelector(
        '#facturesCsvBtn'
    ) as HTMLButtonElement;
    const integrityBtn = searchRow.querySelector(
        '#facturesIntegrityBtn'
    ) as HTMLButtonElement;
    const integrityResultDiv = searchRow.querySelector(
        '#facturesIntegrityResult'
    ) as HTMLDivElement;
    ...
    function pintarTaula(data: ApiResponse): void {
      ...
      `;
        ...
        rows.forEach((f: FacturaListado) => {
            ...
            const fechaEmision_format = fechaEmision_date.toLocaleDateString(
                'es-ES',
                opcionesFormato
            );
      ...
      `;
            ...
            const btnFacturaPdf = tr.querySelector(
                '.factura-pdf'
            ) as HTMLAnchorElement | null;
            if (btnFacturaPdf) {
                btnFacturaPdf.addEventListener(
                    'click',
                    async (e: MouseEvent) => {
                        e.preventDefault();
                        ...
                        try {
                            const pdfUrl =
                                await emitirFacturaYObtenerPdfUrl(rid);
                            window.open(pdfUrl, '_blank', 'noopener');
                        } catch (error: unknown) {
                            console.error('Error al generar el PDF:', error);
                            alert(
                                'Hubo un error al generar la factura. Intenta de nuevo.'
                            );
                        } finally {
