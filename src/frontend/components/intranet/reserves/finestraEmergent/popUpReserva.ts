...
function applyVisibilityRules(
    ...
    const btnConfirm = container.querySelector(
        '#enlace1'
    ) as HTMLElement | null;
    const btnFactura = container.querySelector(
        '#enlace2'
    ) as HTMLElement | null;
    const btnCancelar = container.querySelector(
        '#enlace4'
    ) as HTMLElement | null;
...
function renderDeviceInfoBlock(
    container: HTMLElement,
    data: DeviceInfoInput
): void {
    const info: DeviceInfo = Array.isArray(data)
        ? (data[0] ?? {})
        : (data ?? {});
...
function setBtnState(
    ...
    opts: {
        text: string;
        disabled: boolean;
        klass: 'secondary' | 'warning' | 'success' | 'danger';
    }
): void {
    ...
    if (btnConfirm instanceof HTMLButtonElement) {
        ...
        clone.addEventListener('click', async (ev: MouseEvent) => {
            ...
            try {
                ...
                const msg =
                    err instanceof Error ? err.message : 'Error enviant email';
                setBtnState(clone, {
                    text: 'Error enviant (Reintentar)',
                    disabled: false,
                    klass: 'danger',
                });
                console.error(msg);
            ...
            clone.addEventListener('click', async (ev: MouseEvent) => {
                ...
                try {
                    ...
                    window.dispatchEvent(
                        new CustomEvent('reserva:changed', {
                            detail: { id: reservaId, action: 'cancelada' },
                        })
                    );
    ...
    if (btnFactura instanceof HTMLButtonElement) {
        ...
        clone.addEventListener('click', async (ev: MouseEvent) => {
            ...
            try {
                ...
                setBtnState(clone, {
                    text: 'Error enviando (Reintentar)',
                    disabled: false,
                    klass: 'danger',
                });
                console.error(msg);
...
export function initPopupReservaUX(): void {
    ...
    document.addEventListener('click', (ev: MouseEvent) => {
        ...
        if (isHTMLElement(target) && target.closest('.obrir-finestra-btn'))
            return;
...
async function cancelarReserva(reservaId: string): Promise<string> {
    ...
    if (!res.ok || data.status !== 'success') {
        throw new Error(
            data.message || data.code || `Error cancelando (HTTP ${res.status})`
        );
    }
