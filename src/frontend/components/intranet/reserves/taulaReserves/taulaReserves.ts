...
export async function confirmarPagoManual(
    ...
    if (!res.ok || json.status !== 'success') {
        throw new Error(
            json.message || json.code || 'Error confirmando pago manual'
        );
    }
...
const getOrCreateTableBody = (): HTMLTableSectionElement => {
    ...
    if (!container) {
        throw new Error(
            "No se ha encontrado el contenedor con id 'contenidorTaulaReserves'"
        );
    }

    // ¿Ya existe la tabla?
    let table = container.querySelector(
        '#taulaReserves'
...
export const carregarDadesTaulaReserves = async (
    estatParking: string,
    tipo?: string
): Promise<void> => {
    try {
        ...
        if (!isApiOk(payload)) {
            throw new Error(
                `${payload.code ?? 'API_ERROR'}: ${payload.message}`
            );
        }
        ...
        datos.forEach((data: Reserva) => {
            ...
            const fechaReserva_formateada = fechaReservaDate.toLocaleDateString(
                'es-ES',
                opcionesFormato
            );
            ...
            const dataEntrada2 = dataEntradaDate.toLocaleDateString(
                'es-ES',
                opcionesFormato2
            );
            const dataEntradaAny = dataEntradaDate.toLocaleDateString(
                'es-ES',
                opcionesFormato3
            );
            ...
            const dataSortida2 = dataSortidaDate.toLocaleDateString(
                'es-ES',
                opcionesFormato2
            );
            const dataSortidaAny = dataSortidaDate.toLocaleDateString(
                'es-ES',
                opcionesFormato3
            );
            ...
            if (data.estado === 'anual') {
                html +=
                    '<button type="button" class="btn btn-primary btn-sm">Client anual</button>';
            } else {
          ...
          ${data.factura_numero}/${data.factura_serie}
            ...
            const diffDays = Math.floor(
                (ahora.getTime() - fechaReserva.getTime()) / MS_PER_DAY
            );
          ...
          <button class="btn btn-success btn-sm obrir-finestra-btn"
            ...
            const btnCheckIn = fila.querySelector(
                '.js-check-in'
            ) as HTMLButtonElement | null;
            const btnCheckOut = fila.querySelector(
                '.js-check-out'
            ) as HTMLButtonElement | null;
            ...
            if (btnCheckIn) {
                btnCheckIn.addEventListener('click', async () => {
                    try {
                        ...
                        await carregarDadesTaulaReserves(
                            estatParking,
                            tipo_int
                        );
                    } catch (error) {
            ...
            const btnFacturaPdf = fila.querySelector(
                '.factura-pdf'
                ...
                btnFacturaPdf.addEventListener('click', async (e) => {
                    ...
                    try {
                        ...
                        if (payload.status === 'success') {
                            const pdfUrl = payload.data?.pdf_url?.replace(
                                /\\\//g,
                                '/'
                            );
                            if (pdfUrl) {
            ...
            const btnConfirmarPago = fila.querySelector(
            ...
            if (btnConfirmarPago) {
                btnConfirmarPago.addEventListener('click', async () => {
                    const reservaId = Number(
                        btnConfirmarPago.getAttribute('data-id') ?? 0
                    );
                    if (!reservaId) return;

                    const ok = confirm(
                        '¿Confirmar pago manual y generar factura?'
                    );
                    if (!ok) return;
                    ...
                    try {
                        ...
                        alert(
                            facturaId
                                ? `OK. Factura generada (ID ${facturaId}).`
                                : 'OK. Pago confirmado.'
                        );
