...
function renderBody(container: HTMLElement): void {
    const tbody =
        container.querySelector<HTMLTableSectionElement>('#usersTbody');
    ...
    if (!tbody || !count || !pageInfo || !prevBtn || !nextBtn || !roleButtons)
        return;
    ...
    pageInfo.innerHTML =
        total === 0
            ? `Pàgina 0/0`
            : `Pàgina <strong>${currentPage}</strong>/<strong>${totalPages}</strong>`;
...
function renderRoleButtons(): string {
    const btn = (key: ActiveRole, label: string): string => {
        const activeClass =
            key === activeRole ? 'btn-primary' : 'btn-outline-primary';
...
function renderRows(rowsToRender: ApiUserRow[]): string {
    ...
    return rowsToRender
        .map((r) => {
            ...
            const roleBadge = escapeHtml(
                String(normalizeRoleUiLabel(r.tipo_rol))
            );
...
async function safeLoad(container: HTMLElement): Promise<void> {
    try {
        ...
        const count = container.querySelector<HTMLDivElement>('#usersCount');
        if (tbody)
            tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-4">Error carregant dades: ${escapeHtml(msg)}</td></tr>`;
