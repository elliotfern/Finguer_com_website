...
export async function formClientAnual(isUpdate: boolean, uuid?: string) {
    ...
    if (!isUpdate) {
        ...
        if (!uuid) {
            setTitle(
                `<h5>Clients anuals: modificació dades</h5><p>Falta UUID per a carregar l'usuari.</p>`
            );
            btn.disabled = true;
            return;
        }
        ...
        if (!res || !isOk(res)) {
            setTitle(
                `<h2>Clients anuals: modificació</h2><p>No s'ha pogut carregar les dades de l'usuari.</p>`
            );
            btn.disabled = true;
            return;
        }
