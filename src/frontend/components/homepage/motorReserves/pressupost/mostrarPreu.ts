...
): CotizarLinea | undefined {
    return lineas.find((l) => codigos.includes(l.codigo));
}

export function mostrarPreu(data: CotizarResponse): void {
    setText(
        'resumenReserva',
        `Duración de la reserva: ${data.diasReserva} días`,
        true
    );

    const lineas = data.lineas;

    const tarifa = findLinea(lineas, [
        'RESERVA_FINGUER',
        'RESERVA_FINGUER_GOLD',
    ] as const);
    const limpieza = findLinea(lineas, [
        'LIMPIEZA_EXT',
        'LIMPIEZA_EXT_INT',
        'LIMPIEZA_PRO',
    ] as const);
    ...
    if (tarifa)
        setText('costeReserva', `Reserva: ${eur(tarifa.base)} (sin IVA)`, true);
    else setText('costeReserva', '', false);

    if (seguro)
        setText('costeSeguro', `Seguro: ${eur(seguro.base)} (sin IVA)`, true);
    else setText('costeSeguro', `Seguro: No contratado`, true);

    if (limpieza)
        setText(
            'costeLimpieza',
            `Limpieza: ${eur(limpieza.base)} (sin IVA)`,
            true
        );
