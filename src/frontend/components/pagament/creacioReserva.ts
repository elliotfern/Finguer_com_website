...
function hexToUuid(hex: string): string {
    const clean = hex.toLowerCase().replace(/-/g, '');
    return [
        clean.slice(0, 8),
        clean.slice(8, 12),
        clean.slice(12, 16),
...
export const creacioReserva = async (
    ...
    if (!session) {
        return {
            status: 'error',
            message: 'No se pudo determinar la sesión del carrito.',
        };
    }

    // Campos del formulario (usuario)
    const vehiculo =
        (document.getElementById('vehiculo') as HTMLInputElement | null)
            ?.value || '';
    const matricula =
        (document.getElementById('matricula') as HTMLInputElement | null)
            ?.value || '';
    const vuelo =
        (document.getElementById('vuelo') as HTMLInputElement | null)?.value ||
        '';
    const numeroPersonas =
        (document.getElementById('numero_personas') as HTMLInputElement | null)
            ?.value || '';

    // Enviamos SOLO lo necesario. El backend calcula/importa todo desde carro_compra por session.
    const formData = {
        usuario_uuid: hexToUuid(usuarioUuidHex), // formato UUID con guiones
        localizador: localizador, // tu order Redsys (mdHis), ya generado antes de este paso
    ...
    try {
        ...
        const messageOk = document.querySelector(
            '#messageOk'
        ) as HTMLElement | null;
