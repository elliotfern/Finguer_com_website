...
function requireMethod(string $method): void
{
    ...
    // Preflight CORS: siempre se responde 204, sin exigir el método esperado
    if ($actual === 'OPTIONS') {
...
function requireAuthTokenCookie(): string
{
    ...
    if ($token === '') {
        jsonResponse(
            vp2_err('Access not allowed (no token)', 'UNAUTHENTICATED'),
            401,
        );
    }
...
function readJsonBody(bool $required = false): array
{
    ...
    if (!$raw) {
        if ($required) {
            jsonResponse(vp2_err('Missing JSON body', 'MISSING_BODY'), 400);
        }
        return [];
    }
...
function getIntParam(string $key, bool $required = true): ?int
{
    ...
    if ($v === false || $v === null) {
        if ($required) {
            jsonResponse(
                vp2_err("Parámetro {$key} inválido", 'BAD_PARAM'),
                400,
            );
        }
        return null;
