...
function verificarPagament(int $reservaId, array $opts = []): array
{
    ...
    $repo = new ReservaRepository($conn);
    $p1 = $repo->findById($reservaId);
    ...
    if (!empty($opts['crear_factura'])) {
        $facturaId = crearFacturaParaReserva(
            $conn,
            (int) $reserva['id'],
            'redsys',
        );
        ...
        $facturaId = $repo->obtenerFacturaIdPorReserva((int) $reserva['id']);
    ...
    return vp2_ok('Pago confirmado y flujo completado.', $data, [
        'step' => !empty($opts['enviar_factura'])
            ? 6
