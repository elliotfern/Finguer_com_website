...
function verificarPagament(int $reservaId, array $opts = []): array
{
    ...
    $opts = array_merge([
        'solo_info'           => true,   // por defecto solo informa
        'actualizar_bd'       => false,
    ...
    // 1) cargar reserva y localizador
    ...
    // Si no está pagado, terminamos aquí (solo info)
    if (!$paid) {
        return vp2_ok(
            $p2['message'] ?? 'Redsys: pago no confirmado.',
            [
                'reserva' => $reserva,
                'redsys'  => $redsys,
                'opts'    => $opts,
    ...
    // Si está pagado pero NO queremos tocar BD, terminamos aquí (solo info)
    if (empty($opts['actualizar_bd'])) {
        return vp2_ok(
            $p2['message'] ?? 'Redsys: pago confirmado.',
            [
                'reserva' => $reserva,
                'redsys'  => $redsys,
                'opts'    => $opts,
    ...
    // -------------------
    // STEP 3: registrar cobro + marcar reserva pagada (idempotente)
    // -------------------
    $p3 = registrarCobroConfirmado($conn, (int)$reserva['id'], [
        'metodo'     => 'tarjeta',
        'pasarela'   => 'REDSYS',
        'referencia' => (string)$reserva['localizador'], // Ds_Order
        'importe'    => null, // usa total_calculado
    ]);

    if (($p3['status'] ?? '') !== 'success') {
        return array_merge($p3, [
            'step' => 3,
            'data' => array_merge($p3['data'] ?? [], [
                'reserva' => $reserva,
                'redsys'  => $redsys,
                'opts'    => $opts,
            ]),
    ...
    // -------------------
    // STEP 4: enviar confirmación (modo CRON => no reenvía si ya se envió)
    // -------------------
    if (!empty($opts['enviar_confirmacion'])) {
        $p4 = enviarConfirmacionReserva($conn, (int)$reserva['id'], [
            'force' => false, // CRON-safe
        ]);

        $data['confirmacion'] = $p4;

        // Si quieres que un fallo de email rompa el proceso, cambia esto.
        // Yo recomiendo NO romper: pago ya está registrado.
    ...
    // OK final (step 3 o step 4)
    return vp2_ok(
        !empty($opts['enviar_confirmacion'])
            ? 'Pago confirmado, BD actualizada y confirmación gestionada.'
            : 'Pago confirmado (Redsys) y BD actualizada (pago + reserva pagada).',
        $data,
        ['step' => !empty($opts['enviar_confirmacion']) ? 4 : 3]
    );
}
...
// 3) si pagado:
//    - si actualizar_bd => registrarPagoConfirmadoYMarcarReservaPagada(...)
//    - si enviar_confirmacion => enviarConfirmacio(...)
//    - si crear_factura => $facturaId = crearFacturaParaReserva(...)
//    - si enviar_factura => enviarFactura($facturaId)
...
function enviarFactura($idFactura)
{
    ...
    if (!$row) {
        return ['status' => 'error', 'message' => 'No se ha seleccionado ninguna factura.'];
    }
    ...
    $baseDir = '/home/epgylzqu/finguer.com'; // path real en servidor
    $dir = $baseDir . '/pdf/facturas';

    if (!is_dir($dir)) {
        if (!mkdir($dir, 0755, true) && !is_dir($dir)) {
            return ['status' => 'error', 'message' => 'No se pudo crear carpeta pdf/facturas'];
        }
    }

    $numeroFacturaSafe = $row['serie'] . '_' . $row['numero']; // 2025_00023
    $filename = $dir . '/' . $numeroFacturaSafe . '.pdf';

    $pdf->Output($filename, 'F');

    if (!file_exists($filename) || filesize($filename) === 0) {
        return ['status' => 'error', 'message' => 'PDF no generado o vacío', 'path' => $filename];
    }

    // 4) Enviar email

    if (!filter_var($email_old, FILTER_VALIDATE_EMAIL)) {
        return ['status' => 'error', 'message' => 'Email de cliente inválido', 'email' => $email_old];
    }
