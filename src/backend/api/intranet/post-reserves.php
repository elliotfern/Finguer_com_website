...
if ($type !== 'update-estado') {
    jsonResponse(
        vp2_err('type inválido', 'BAD_TYPE', ['allowed' => ['update-estado']]),
        400,
    );
}
...
$nuevoEstado = isset($input['estado_vehiculo'])
...
try {
    ...
    if ($code === 'BAD_ESTADO') {
        jsonResponse(
            vp2_err('Parámetro estado_vehiculo inválido', 'BAD_ESTADO', [
    ...
    jsonResponse(
        vp2_err('Transición de estado no permitida', 'INVALID_TRANSITION', [
            'id' => $e->id,
            'from' => $e->from,
            'to' => $e->to,
    ...
    jsonResponse(
        vp2_err('Error al actualizar el estado de la reserva', 'SERVER_ERROR', [
