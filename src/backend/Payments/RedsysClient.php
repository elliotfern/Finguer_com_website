...
class RedsysClient
{
    ...
    public function consultaOperacion(
        string $order,
        ...
        if (!$response) {
            return [
                'status' => 'error',
                'message' => 'No response from Redsys',
            ];
        }
    ...
    private function parseResponse(string $xml): array
    {
        ...
        if (
            preg_match(
                '/<consultaOperacionesReturn>(.*?)<\/consultaOperacionesReturn>/s',
                $xml,
                $matches,
            )
        ) {
            ...
            return [
                'status' => 'error',
                'message' => 'No consultaOperacionesReturn in response',
                'raw' => $xml,
            ];
        }
